...
Conftest denies a planned root S3 bucket unless exactly one public-access block references exactly that bucket, has a known planned `after.bucket` equal to the bucket's known planned name (exposed through `.bucket`), and enables all four protections. Open or unknown non-ALB ingress is denied; unknown legacy-rule direction is treated as potentially ingress. An ALB exemption requires exactly one distinct security-group reference and a planned root application-ALB instance, with known planned attachment IDs agreeing when available. On a fresh create, a conditional between one planned group and a literal pre-existing group ID remains an accepted plan-time residual because the reference set cannot distinguish it. The gate in `gates` and enforcement on the live `plan-localstack` PR plan are VERIFIED in CI on PR #12's final head; run ids in the PR description, while `session-apply` gating of the saved AWS plan before `make apply` is CODE-ONLY until P0-3d.
