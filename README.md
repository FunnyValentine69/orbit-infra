...
- Lease lifecycle with compare-and-swap on S3, a mandatory owner token on every open and claim, and a two-stage close (destroy-and-verify, then re-probe and remove state).
...
- **AWS-SIMULATED** — 241 custom-policy cases (240 passed, 1 custom-only finding, 0 runner failures) and 158 role-policy cases (158 passed, 0 failed; 155 agreements, 3 divergences); the role lane closes the custom-only finding, and `SnsSubscriptionManage` confirms star-only policy evaluation. See [`docs/assets/IAM_SIMULATION_REPORT.md`](docs/assets/IAM_SIMULATION_REPORT.md). This proves policy evaluation, not service enforcement.
