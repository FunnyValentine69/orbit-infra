...
The suite scans Markdown links and images in every tracked Markdown file, rejects retired paths both as link targets and as tracked files, and applies a case-insensitive scan to tracked Markdown and shell files for the seven forbidden strings. It enforces the six publication line budgets, requires all five front-page embeds, indexes every file under `docs/assets/` and `docs/evidence/`, and confirms the removed state ledger is untracked. It injects and kills 20 mutations: one budget overflow, one broken link, one retired link target, one bare retired matrix link, one bare retired runbook prose reference, one missing evidence entry, one tracked state ledger, one missing publication file, one tracked retired file, one tracked file missing on disk, one missing front-page embed, one absent four-file cartoon asset set, all seven forbidden strings, and one lower-case forbidden-string variant. The restored scanner must pass with no skipped publication checks.
...
Three groups cover generator contracts, generator provenance guards, and committed asset contracts. The generator group proves deterministic cartoon, transcript, and emblem rendering; exact scene timing and boundary snapshots; accessibility and structural hygiene. Fifteen focused checks cover Orbit's gate-layer crossing, centered REC and end-card text, the gate's single-unit rise, door roll-up geometry, door scale formatting, all-child rope parking including quadratic-curve extrema, full proof-card parking, doc-card/Bill clearance, code-card/grid clearance, card/card clearance, coin spawn clearance, seal timing, the seal drop track, Scout's off-canvas cut, and cloud/wipe registration. The provenance-guard group proves both generators must be tracked and clean at `HEAD` before provenance is written, while output overrides remain available for isolated tests. The asset group proves all four outputs exist together, match regenerated bytes and recorded hashes, bind to an ancestor commit containing both generators, and remain linked from the README and evidence index. Across the three groups, the suite kills 47 mutants: 42 in the generator group, two in the provenance-guard group, and three in the asset group. The documentation gate invokes this suite with the asset group enabled after its 20-mutation PASS.
