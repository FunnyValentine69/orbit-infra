...
mkdir -p "$gate_root/scripts" "$gate_root/bootstrap" "$gate_root/modules" \
  "$gate_root/envs" "$gate_root/tests" "$gate_root/bin"
...
exit 0
EOF
cat > "$gate_root/tests/docs-contracts.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
echo docs >> "${DOCS_CONTRACT_CALL_LOG:?}"
echo docs >> "${GATE_CALL_LOG:?}"
if [ "${DOCS_GATE_FAIL:-0}" = 1 ]; then
  echo "injected docs contract failure"
  exit 1
fi
EOF
...
echo policy-size >> "${GATE_CALL_LOG:?}"
EOF
chmod +x "$gate_root/bin/make" "$gate_root/tests/docs-contracts.sh" \
  "$gate_root/bootstrap/policy-size-check.sh"

policy_size_call_log="$tmp_dir/policy-size-calls.log"
docs_contract_call_log="$tmp_dir/docs-contract-calls.log"
gate_call_log="$tmp_dir/gate-calls.log"
: > "$policy_size_call_log"
: > "$docs_contract_call_log"
: > "$gate_call_log"
default_gates_output="$(
  cd "$gate_root"
  env -u GATES_POLICY_SIZE PATH="$gate_root/bin:$PATH" \
    POLICY_SIZE_CALL_LOG="$policy_size_call_log" \
    DOCS_CONTRACT_CALL_LOG="$docs_contract_call_log" \
    GATE_CALL_LOG="$gate_call_log" bash scripts/gates.sh
)"
...
if ! grep -Fq 'PASS: docs-contracts' <<< "$default_gates_output" || \
   [ "$(wc -l < "$docs_contract_call_log" | tr -d ' ')" != 1 ]; then
  echo "gates.sh must run docs-contracts exactly once: $default_gates_output" >&2
  exit 1
fi
expected_gate_order="$tmp_dir/expected-gate-order.log"
cat > "$expected_gate_order" <<'EOF'
make:validate
make:lint
make:test
docs
...
make:conftest
EOF
if ! cmp -s "$expected_gate_order" "$gate_call_log"; then
  echo "gates.sh gate order changed: $(tr '\n' ' ' < "$gate_call_log")" >&2
  exit 1
fi
...
failed_docs_output="$(
  cd "$gate_root"
  PATH="$gate_root/bin:$PATH" \
    POLICY_SIZE_CALL_LOG="$policy_size_call_log" \
    DOCS_CONTRACT_CALL_LOG="$docs_contract_call_log" \
    GATE_CALL_LOG="$gate_call_log" DOCS_GATE_FAIL=1 \
    bash scripts/gates.sh 2>&1
)"
failed_docs_rc=$?
set -e
if [ "$failed_docs_rc" -eq 0 ] || \
   ! grep -Fq 'FAIL: docs-contracts' <<< "$failed_docs_output" || \
   ! grep -Fq 'injected docs contract failure' <<< "$failed_docs_output" || \
   [ "$(wc -l < "$docs_contract_call_log" | tr -d ' ')" != 1 ]; then
  echo "gates.sh must propagate one docs-contracts failure: rc=$failed_docs_rc output=$failed_docs_output" >&2
  exit 1
fi
