...
| IAM action-condition matrix | 220 cases in 65 rows are `AWS-SIMULATED`; all point to `docs/assets/IAM_SIMULATION_REPORT.md` and carry the current publication date; rows containing a live-call-only or not-simulatable case retain their lower label |
| IAM simulation report | `AWS-SIMULATED 2026-09-12 docs/assets/IAM_SIMULATION_REPORT.md`; 241 custom-policy cases (240 passed, 1 custom-only finding, 0 runner failures) and 158 role-policy cases (158 passed, 0 failed; 155 custom-lane agreements, 3 divergences); the role lane closes the custom-only finding, `SnsSubscriptionManage` confirms star-only policy evaluation, 429 Organizations divergences are recorded, and there are zero manual cleanup notes; proves policy evaluation, not service enforcement |
