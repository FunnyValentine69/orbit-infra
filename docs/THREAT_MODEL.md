...
| 2 | Spoofing | Pull-request or non-main code receives AWS credentials. | Pull-request plans use LocalStack; no role trusts the pull-request subject. | Exact trust-condition enforcement remains `CODE-ONLY`. When `oidc-smoke.yml` is enabled, same-repository smoke checks are intentionally negative. The workflow is currently disabled in repository settings; see [pull-request checks](VERIFY.md#pull-request-checks). |
