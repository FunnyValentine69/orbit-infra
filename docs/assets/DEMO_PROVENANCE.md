...
| generator commit | d9d33f6 (the tree at this commit holds every path in DEMO_GENERATOR_PATHS) |
...
| artifact | 222271 bytes, 29.680000 s, 742 frames |
| artifact sha256 | 3e9f6f52b9e96ab5e59319eeab114300aefb4802be104c81390f8d1a2a5e2874 |
...
- Frames: `ffmpeg` decoded all 742 frames of the artifact row (passthrough decode exit 0); 255 unique frames by MD5.
- OCR: tesseract 5.5.3 run on this recording (255 unique frames, same grep set): no matches at all; the visible S3 references are Terraform resource addresses, so the physical bucket name and the project suffix never appear on screen (2026-09-07).
