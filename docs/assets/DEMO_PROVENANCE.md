...
| generator commit | 2143783 (the tree at this commit holds the `demo/demo.tape`, `demo/record.sh` and `Makefile` targets that produced the recording) |
...
| artifact | 233900 bytes, 31.84 s, 796 frames |
...
- Frames: `ffmpeg -fps_mode passthrough` decoded 796 frames, equal to the source count of 796; 297 unique frames by SHA-256.
...
- Viewed: 12 frames viewed by the reviewer (every section boundary plus a spread sample) and one frame per section by the orchestrator.
