...
| generator commit | eced69b (the tree at this commit holds every path in DEMO_GENERATOR_PATHS) |
...
| artifact | 224215 bytes, 30.160000 s, 754 frames |
| artifact sha256 | b91915ea3abb634f5adc4c2c9cd279bd5863a6d6534e633601effd75607e787a |
...
- Frames: `ffmpeg` decoded all 754 frames of the artifact row (passthrough decode exit 0); 277 unique frames by MD5.
- OCR: tesseract 5.5.3 run on this recording (277 unique frames, same grep set): no matches at all; the visible S3 references are Terraform resource addresses, so the physical bucket name and the project suffix never appear on screen (2026-09-07).
