...
| generator commit | 278a7d2 (the tree at this commit holds every path in DEMO_GENERATOR_PATHS) |
...
| artifact | 218733 bytes, 28.560000 s, 714 frames |
| artifact sha256 | a0ba5da0e5cc2b5645fc583e025e5904a60b9ef7329f6763bea5742997690460 |
...
- Frames: `ffmpeg` decoded all 714 frames of the artifact row (passthrough decode exit 0); 238 unique frames by MD5.
- OCR: tesseract 5.5.3 run on this recording (238 unique frames, same grep set): no matches at all; the visible S3 references are Terraform resource addresses, so the physical bucket name and the project suffix never appear on screen (2026-09-07).
