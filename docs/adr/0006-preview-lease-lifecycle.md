...
read and CAS. The sweeper never supplies `--force-retry`. Its Stage 2 claim may
take over a claim older than the 60-minute job timeout by a further 60-minute
safety margin; the fresh-read CAS records the cleared token and claim time in
`cleanup_retry_audit` before installing the replacement.
...
recognized status; any other response is indeterminate and invokes
`fail-stage2`, leaving the lease `closing` with its claim cleared. The exact
deleted response remains an authored
...
`envs/preview/<env_id>.tfstate` key and its exact `.tflock` sibling, deletes
them in batches of at most 1,000, and performs a final empty-version read. A
partial or indeterminate deletion invokes `fail-stage2`, increments
`stage2_attempt`, leaves the lease `closing`, and clears the matching claim. `closed` is
...
Prune re-reads a `closed` lease older than seven days and replaces it under the
same ETag precondition with a minimal `deleted` tombstone containing the
environment ID, generation, and lifecycle timestamps. `open` on that tombstone
continues at generation N+1; the one-object-per-environment footprint remains
constant.
...
Stage 1 retries `pending`, post-destroy `live`, and transient `indeterminate` results for up to five minutes, with backoff capped at 30 seconds. Partial results are appended to `manifest.verification_runs` on every iteration. A non-zero verifier exit or malformed verifier result is routed through the same `cleanup_failed` lease transition with a recorded error.
Before using a verifier result, close validates the outcome vocabulary, recomputes all four counts from `results`, requires the reported summary to match, and requires `passed` to equal the verifier rule (`live == 0 && indeterminate == 0`; pending is left for Stage 2). If present, `stale_tag_entries` must be an array of objects. Any discrepancy records `cleanup_failed`, and close decisions use only the recomputed counts plus the validated `passed` value.
At the deadline, `live` or `indeterminate` results set `cleanup_failed`; `pending` may remain for Stage 2. Stale tag records whose exact probes are `gone` are persisted under `manifest.stale_tag_entries` and never fail Stage 1.
...
Each lease generation stores `cleanup_attempt`, `stage2_attempt`, `next_retry_at`, and `manual_intervention_required` through the same ETag compare-and-swap as status and manifest updates. Three automatic Stage 1 executions and three automatic Stage 2 executions are permitted; each stage escalates on its own counter.
The third Stage 1 failure retains `cleanup_failed`.
The third Stage 2 failure retains `closing`; both stages retain state and require manual intervention. A fourth automatic claim is refused. An operator may use the explicit `--force-retry` path after review, which clears any active Stage 2 claim and appends that cleared claim to the lease audit entry.
...
generation, and that token. `complete-stage1` clears it by CAS. `claim-stage2` requires an unclaimed
`closing` lease unless `--takeover-stale` proves the existing Stage 2 claim old
enough in the same fresh read; `release-stage2` relinquishes only a matching
pending claim. `fail-stage2` requires that same token, increments
`stage2_attempt`, and clears it without changing `cleanup_attempt` or
`next_retry_at`. Generic `transition` accepts only the Stage 1
`cleanup_failed` path and refuses claim ownership from the other stage.
Only `complete-stage2` may
produce `closed`, in the same CAS that appends the Stage 2 proof and clears its
claim. Opening a `closed` lease or its `deleted` tombstone advances the retained
generation.
