...
(expanded when the phase starts)
- [x] P5-1b: Stage-2 sweeper (`scripts/sweep.sh`, AWS-only nightly `sweeper.yml` matrix, in-job LocalStack Stage 2, version/delete-marker state removal, ETag-conditioned prune, `tests/sweeper.sh` fixture suite) — LOCALSTACK-VERIFIED locally 2026-09-03 (env sw1: apply 59, stage-1 close, `SWEEP_IN_JOB=true scripts/sweep.sh env sw1` → 3 task definitions deleted-by-allowance, state versions and delete markers removed, lease `closed` with `stage2_runs[-1].in_job`; second sweep no-op within the retention window; reopen gave generation 2); CODE-ONLY until promotion for:
