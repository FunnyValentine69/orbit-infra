...
- [x] P4-1: repeatable concurrency script `tests/localstack-concurrency.sh` (two environments apply/close concurrently, independent state and leases) — LOCALSTACK-VERIFIED locally 2026-09-03 (`make test-concurrency`: environments=2, state_resources=126, tagged_resources=60, residual_inventory_entries=44 all classified gone by the verifier's exact probes, lease_refusals=4); the first run exposed a test defect (the tagging API retains deleted resources on LocalStack, so the post-close predicate now defers entirely to the verifier) fixed before the passing run; the CI-mode execution stays CODE-ONLY until the first main-only dispatch
- [x] P4-2: three-dispatch ordering test `tests/dispatch-ordering.sh` (two same-environment applies plus target-specific destroy behavior) and the exact post-merge PR-comment commands — CODE-ONLY; after PR #5 merges, promote with `ENV_ID=ord1 TARGET=localstack REF=main bash tests/dispatch-ordering.sh`
- [x] P4-3: numbered RUNBOOKS procedures for start-session, end-session, stuck-environment force-destroy, re-signing an already-signed digest, operator CIDR change, rotate secrets, and image bump — end-session and stuck-environment force-destroy are LOCALSTACK-VERIFIED (2026-09-03: exhausted `rbstuck` lease at cleanup_attempt 3 refused the automatic retry with exit 3, the audited `--force-retry --generation` run destroyed 59 resources and left `closing` with cleanup_attempt 4 and one forced audit entry); the remaining sections stay CODE-ONLY with the promotion command on each `Executed:` line
